...
#if defined(NDEBUG)
# error "Saolacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6d960dd54bf776b2af3eefb46812fbdecdf6d80168a2d7e8052e8243fb76a6dd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Saolacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Saolacoin Signed Message:\n";
...
{
    // Saolacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 21000000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 21000000); // Saolacoin: 21000k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Saolacoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Saolacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Saolacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Saolacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf903b0f26086c75a5f5dd99e3eb893287f888c580e411afead6aa3eb7fbd689b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 5/26/2021 - Saolacoin hits over a million views!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04521395ceded411823cbded72f57efa674f2950292cf9ef34902e7277a814fd81d30e96d1bae16d356f04d8eeb7c27c5e9e1df1491a2f71004660282d24ec5a50") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622091061;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085066187;

        if (fTestNet)
        {
            block.nTime    = 1622091031;
            block.nNonce   = 386857932;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe4fdd48b9c03771d15fed841624853c0a70abb1ed88201ea905b8b27155f611d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Saolacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SaolacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SaolacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SaolacoinMiner(CWallet *pwallet)
{
    printf("SaolacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("saolacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SaolacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SaolacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SaolacoinMiner, pwallet));
}
